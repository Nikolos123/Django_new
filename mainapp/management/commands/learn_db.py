...
class Command(BaseCommand):
    def handle(self, *args, **options):
        ...
        action_1_timedelte = timedelta(hours=12)
        action_2_timedelte = timedelta(hours=24)

        action_1_discount = 0.3
        action_2_discount = 0.15
        action_3_discount = 0.05

        action_1_condition = Q(order__updated__lte=F('order__created') + action_1_timedelte)
        action_2_condition = Q(order__updated__gte=F('order__created') + action_1_timedelte,
                               order__updated__lte=F('order__created') + action_2_timedelte)

        action_3_condition = Q(order__updated__gte=F('order__created') + action_2_timedelte)
        ...
        test_orderss = OrderItem.objects.annotate(
            ...
            total_price=Case(
                ...
                action_2_price,
                action_3_price,
                output_field=DecimalField(), )).order_by('action_order', 'total_price').select_related()
